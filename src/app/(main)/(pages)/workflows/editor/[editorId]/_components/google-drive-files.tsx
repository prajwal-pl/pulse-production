...
const GoogleDriveFiles = (props: Props) => {
  ...
  const stopListener = async () => {
    setLoading(true);
    try {
      const response = await axios.delete("/api/drive-activity");
      toast.message(response.data.message);
      setIsListening(false);
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Failed to stop listener");
    } finally {
      setLoading(false);
    }
  };
          ...
          >
            {loading ? (
              <div className="flex items-center gap-2">
                <svg
                  aria-hidden="true"
                  className="inline h-4 w-4 animate-spin fill-white text-gray-200"
                  viewBox="0 0 100 101"
                  fill="none"
